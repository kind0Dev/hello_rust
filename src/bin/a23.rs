...
fn part_1() -> bool {
    ...
    let has_access = maybe_access("admin");
    has_access.is_some()
...
fn part_2() -> Option<Access> {
    ...
    let has_access = maybe_access("root");
    has_access.or_else(||root())
}

fn part_3() -> Access {
    // "Alice" is not a listed user, so she will be a guest.
    // Note: Use unwrap_or_else.
    let has_access = maybe_access("Alice");
    has_access.unwrap_or_else(||Access::Guest)
}
